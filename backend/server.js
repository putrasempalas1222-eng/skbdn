...
const emailLimiter = rateLimit({
  ...
  message: {
    ...
    message: 'Please wait before sending more notification emails.'
  },
});
...
app.post('/api/notifications/email', emailLimiter, async (req, res) => {
  ...
  if (!transporter) {
    return res.status(503).json({
      ...
      message: 'Set SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS, and SMTP_FROM in backend/.env.local.'
    });
  }
...
app.post('/api-proxy', async (req, res) => {
  if (!isGoogleProxyConfigured) {
    return res.status(503).json({
      error: 'Vertex proxy is not configured.',
      message: 'Set GOOGLE_CLOUD_PROJECT, GOOGLE_CLOUD_LOCATION, and PROXY_HEADER to enable this route.'
    });
  }
